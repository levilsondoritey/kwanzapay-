...
// ===== MONGODB =====
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/kwanzapay';
mongoose.connect(MONGO_URI)
  .then(() => { console.log('✅ MongoDB conectado'); setTimeout(seedData, 1500); })
...
const blogSchema = new mongoose.Schema({
  author: String, message: String, image: String,
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await Transaction.create({
        ...
        description: `Comissão nível ${i+1} de ${user.nickname}`
      });
      current = ref;
    }
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (inviteCode) {
      ...
      if (!inviter) return res.status(400).json({ success: false, message: 'Código inválido' });
...
app.post('/api/vip/subscribe', protect, async (req, res) => {
  try {
    ...
    // VALIDAÇÃO: Precisa ter feito depósito antes
    if (!user.hasEverDeposited) {
      return res.status(400).json({ success: false, message: 'Realize um depósito antes de comprar um plano VIP' });
    }
    
    if (user.depositWallet < plan.price) return res.status(400).json({ success: false, message: 'Saldo insuficiente. Faça um depósito' });
    if (user.vipLevel >= level) return res.status(400).json({ success: false, message: 'Você já tem este VIP ou superior' });
    ...
    await Transaction.create({
      user: user._id, type: 'vip_purchase', amount: -plan.price,
      balanceBefore: before, balanceAfter: user.depositWallet,
...
app.get('/api/tasks/today', protect, async (req, res) => {
  ...
  // VALIDAÇÃO: Precisa ter feito depósito
  if (!user.hasEverDeposited) {
    return res.json({ success: false, requireDeposit: true, message: 'Realize um depósito para acessar as tarefas' });
  }
  
  if (user.vipLevel === 0) {
    return res.json({ success: false, requireVip: true, message: 'Compre um plano VIP para realizar tarefas' });
  }
  ...
  for (let i = 1; i <= plan.tasksPerDay; i++) {
    tasks.push({
      number: i,
      reward: plan.rewardPerTask,
      completed: i <= user.tasksCompletedToday
    });
  }
...
app.post('/api/tasks/complete', protect, async (req, res) => {
  try {
    ...
    if (user.tasksCompletedToday >= plan.tasksPerDay) {
      return res.status(400).json({ success: false, message: 'Todas tarefas concluídas. Volte amanhã!' });
    }
    ...
    await Transaction.create({
      user: user._id, type: 'task_reward', amount: plan.rewardPerTask,
      balanceBefore: before, balanceAfter: user.withdrawWallet,
      description: `Tarefa ${user.tasksCompletedToday}/${plan.tasksPerDay}`
    });
    
    res.json({
      success: true,
      message: `+${plan.rewardPerTask} Kz`,
      reward: plan.rewardPerTask,
      completed: user.tasksCompletedToday,
      total: plan.tasksPerDay,
      remaining: plan.tasksPerDay - user.tasksCompletedToday
    });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/api/deposit/payment-info/:method', protect, async (req, res) => {
  ...
  if (method === 'entity') return res.json({
    success: true,
    entity: settings.entityReference.entity,
    references: settings.entityReference.references.filter(r => r.active),
    instructionImage: settings.entityReference.instructionImage,
    instructions: settings.entityReference.instructions
  });
  if (method === 'kwik') return res.json({
    success: true,
    numbers: settings.kwik.numbers.filter(n => n.active),
    instructionImage: settings.kwik.instructionImage,
    instructions: settings.kwik.instructions
  });
...
app.post('/api/deposit/create', protect, async (req, res) => {
  try {
    ...
    if (amount < settings.minDeposit || amount > settings.maxDeposit)
      return res.status(400).json({ success: false, message: `Valor entre ${settings.minDeposit} e ${settings.maxDeposit}` });
    
    let paymentInfo = {};
    if (method === 'bank') {
      const bank = settings.banks.id(bankId);
      if (!bank) return res.status(400).json({ success: false, message: 'Banco inválido' });
      const ibans = bank.ibans.filter(i => i.active);
      if (!ibans.length) return res.status(400).json({ success: false, message: 'Sem IBANs' });
      ...
      if (!refs.length) return res.status(400).json({ success: false, message: 'Sem referências' });
      ...
      if (!nums.length) return res.status(400).json({ success: false, message: 'Sem KWIK' });
...
app.post('/api/deposit/upload-proof', protect, upload.single('proof'), async (req, res) => {
  try {
    ...
    await deposit.save();
    await Notification.create({ type: 'deposit', title: 'Novo Comprovante', message: `${req.user.nickname}: ${deposit.amount} Kz`, forAdmin: true });
...
app.post('/api/deposit/mark-paid', protect, async (req, res) => {
  ...
  if (!deposit) return res.status(404).json({ success: false });
  await Notification.create({ type: 'deposit', title: 'Aguardando aprovação', message: `${req.user.nickname}: ${deposit.amount} Kz`, forAdmin: true });
...
app.post('/api/withdraw/create', protect, async (req, res) => {
  try {
    ...
    const settings = await Settings.findOne();
    if (!amount || !bank || !iban || !accountName || !withdrawPassword)
      return res.status(400).json({ success: false, message: 'Preencha tudo' });
    if (amount < settings.minWithdraw || amount > settings.maxWithdraw)
      return res.status(400).json({ success: false, message: `Valor entre ${settings.minWithdraw} e ${settings.maxWithdraw}` });
    ...
    const fee = (amount * settings.withdrawFee) / 100;
...
app.get('/api/team/info', protect, async (req, res) => {
  ...
  const commissions = await Transaction.aggregate([
    { $match: { user: user._id, type: 'commission' } },
  ...
  res.json({
    ...
    data: {
      inviteCode: user.inviteCode,
      inviteLink: `${process.env.SITE_URL || 'http://localhost:5000'}/register.html?ref=${user.inviteCode}`,
...
app.post('/api/blog/create', protect, upload.single('image'), async (req, res) => {
  const blog = await Blog.create({
    ...
    user: req.user._id, approved: false
  });
...
app.get('/api/admin/dashboard', protect, adminOnly, async (req, res) => {
  ...
  res.json({
    ...
    stats: {
      ...
      totalDeposited: td[0]?.total || 0,
      totalWithdrawn: tw[0]?.total || 0,
...
app.get('/api/admin/users', protect, adminOnly, async (req, res) => {
  ...
  if (search) filter.$or = [
    ...
    { inviteCode: { $regex: search.toUpperCase(), $options: 'i' } }
  ];
  if (status) filter.status = status;
  const users = await User.find(filter).select('-password -withdrawPassword')
    .populate('invitedBy', 'nickname phone').sort({ createdAt: -1 })
...
app.get('/api/admin/users/:id', protect, adminOnly, async (req, res) => {
  ...
  const transactions = await Transaction.find({ user: user._id }).sort({ createdAt: -1 }).limit(30);
  res.json({ success: true, user, deposits, withdrawals, referrals, transactions });
});
...
app.post('/api/admin/users/:id/adjust', protect, adminOnly, async (req, res) => {
  ...
  if (type === 'add') user[field] += parseFloat(amount);
  else { if (user[field] < amount) return res.status(400).json({ success: false, message: 'Saldo insuficiente' }); user[field] -= parseFloat(amount); }
  user.totalAssets = user.depositWallet + user.withdrawWallet;
  await user.save();
  await Transaction.create({
    user: user._id, type: type === 'add' ? 'admin_add' : 'admin_remove',
    amount: type === 'add' ? amount : -amount, balanceBefore: before, balanceAfter: user[field],
    description: note || `Ajuste admin (${wallet})`
  });
...
app.get('/api/admin/deposits', protect, adminOnly, async (req, res) => {
  ...
  const deposits = await Deposit.find(filter).populate('user', 'nickname phone').sort({ createdAt: -1 }).limit(100);
  res.json({ success: true, deposits });
});
...
app.post('/api/admin/deposits/:id/approve', protect, adminOnly, async (req, res) => {
  ...
  if (!d || d.status !== 'pending') return res.status(400).json({ success: false });
  const user = d.user;
  const before = user.depositWallet;
  ...
  user.totalAssets = user.depositWallet + user.withdrawWallet;
  await user.save();
  d.status = 'approved'; d.processedAt = new Date(); d.adminNote = req.body.note || 'Aprovado';
  await d.save();
...
app.post('/api/admin/deposits/:id/reject', protect, adminOnly, async (req, res) => {
  const d = await Deposit.findById(req.params.id).populate('user');
  if (!d || d.status !== 'pending') return res.status(400).json({ success: false });
  d.status = 'rejected'; d.processedAt = new Date(); d.adminNote = req.body.note || 'Rejeitado';
  await d.save();
...
app.get('/api/admin/withdrawals', protect, adminOnly, async (req, res) => {
  const { status } = req.query;
  const filter = status ? { status } : {};
  const withdrawals = await Withdrawal.find(filter).populate('user', 'nickname phone').sort({ createdAt: -1 }).limit(100);
  res.json({ success: true, withdrawals });
});
...
app.post('/api/admin/withdrawals/:id/approve', protect, adminOnly, async (req, res) => {
  ...
  w.status = 'approved'; w.processedAt = new Date(); w.adminNote = req.body.note || 'Pago';
  await w.save();
  w.user.totalWithdrawn += w.amount;
  await w.user.save();
  await Notification.create({ user: w.user._id, type: 'withdraw', title: 'Saque Pago', message: `${w.amount} Kz pago` });
  res.json({ success: true });
});
...
app.post('/api/admin/withdrawals/:id/reject', protect, adminOnly, async (req, res) => {
  ...
  user.totalAssets = user.depositWallet + user.withdrawWallet;
  await user.save();
  w.status = 'rejected'; w.processedAt = new Date(); w.adminNote = req.body.note || 'Rejeitado';
  await w.save();
  ...
  await Notification.create({ user: user._id, type: 'withdraw', title: 'Saque Rejeitado', message: `${w.amount} Kz devolvido. ${w.adminNote}` });
  res.json({ success: true });
});
...
app.post('/api/admin/banks', protect, adminOnly, async (req, res) => {
  ...
  if (s.banks.find(b => b.bankCode === req.body.bankCode)) return res.status(400).json({ success: false, message: 'Já existe' });
  s.banks.push({ ...req.body, ibans: [], active: true });
  await s.save();
  res.json({ success: true });
});
app.put('/api/admin/banks/:bankId', protect, adminOnly, async (req, res) => {
  const s = await Settings.findOne();
  Object.assign(s.banks.id(req.params.bankId), req.body);
  await s.save();
  res.json({ success: true });
});
...
app.post('/api/admin/banks/:bankId/ibans', protect, adminOnly, async (req, res) => {
  ...
  if (bank.ibans.length >= 6) return res.status(400).json({ success: false, message: 'Máx 6 IBANs' });
  bank.ibans.push({ ...req.body, active: true });
  await s.save();
  res.json({ success: true });
});
app.put('/api/admin/banks/:bankId/ibans/:ibanId', protect, adminOnly, async (req, res) => {
  const s = await Settings.findOne();
  Object.assign(s.banks.id(req.params.bankId).ibans.id(req.params.ibanId), req.body);
  await s.save();
  res.json({ success: true });
});
...
app.put('/api/admin/entity-reference', protect, adminOnly, async (req, res) => {
  let s = await Settings.findOne(); if (!s) s = new Settings({});
  Object.assign(s.entityReference, req.body);
  await s.save();
  res.json({ success: true });
});
app.post('/api/admin/references', protect, adminOnly, async (req, res) => {
  const s = await Settings.findOne();
  s.entityReference.references.push({ reference: req.body.reference, active: true });
  await s.save();
  res.json({ success: true });
});
...
app.put('/api/admin/kwik', protect, adminOnly, async (req, res) => {
  let s = await Settings.findOne(); if (!s) s = new Settings({});
  Object.assign(s.kwik, req.body);
  await s.save();
  res.json({ success: true });
});
app.post('/api/admin/kwik/numbers', protect, adminOnly, async (req, res) => {
  const s = await Settings.findOne();
  s.kwik.numbers.push({ ...req.body, active: true });
  await s.save();
  res.json({ success: true });
});
...
app.post('/api/admin/broadcast', protect, adminOnly, async (req, res) => {
  ...
  await Notification.insertMany(users.map(u => ({ user: u._id, type: 'system', title, message })));
  res.json({ success: true, message: `Enviado a ${users.length} usuários` });
});
...
app.post('/api/admin/blogs', protect, adminOnly, async (req, res) => {
  const blog = await Blog.create({ ...req.body, approved: true });
  res.json({ success: true, blog });
});
  ...
  try {
    ...
    if (!await Settings.findOne()) {
      await Settings.create({
        siteName: 'KwanzaPay',
        banks: [
          { bankCode: 'BFA', bankName: 'Banco BFA', logo: '', ibans: [], active: true },
          { bankCode: 'BAI', bankName: 'Banco BAI', logo: '', ibans: [], active: true },
          { bankCode: 'BIC', bankName: 'Banco BIC', logo: '', ibans: [], active: true },
          { bankCode: 'ATL', bankName: 'Banco ATL', logo: '', ibans: [], active: true },
          { bankCode: 'BCI', bankName: 'Banco BCI', logo: '', ibans: [], active: true }
        ],
        withdrawBanks: [
...
A KwanzaPay nasceu para revolucionar a forma como os angolanos ganham dinheiro online. Somos uma plataforma 100% digital, segura, regulamentada e criada por angolanos, para angolanos.

🚀 NOSSA MISSÃO
Proporcionar a cada cidadão angolano a chance de gerar renda extra realizando tarefas simples diárias, com pagamentos rápidos, transparentes e suporte 24/7.

💎 POR QUE ESCOLHER KWANZAPAY?
✅ Tarefas rápidas e fáceis de completar
✅ Pagamentos diários garantidos
✅ Saques processados em apenas 24 horas
✅ Suporte ao cliente em português 24/7
✅ Sistema de convite com comissões generosas (até 30%)
✅ Mais de 48.000 angolanos ativos e ganhando
✅ Parcerias com BFA, BAI, BIC, ATL, BCI
✅ Tecnologia segura com criptografia bancária
✅ Sistema VIP transparente e justo

🏆 NOSSO IMPACTO
Já pagamos mais de 5 bilhões de Kwanzas aos nossos usuários, transformando milhares de vidas em todas as 21 províncias de Angola.

🎯 JUNTE-SE A NÓS
Faça parte da maior comunidade financeira digital de Angola. Cadastre-se agora, faça seu primeiro depósito e comece a ganhar dinheiro hoje mesmo!

🇦🇴 KwanzaPay — Plataforma Angolana. Feito para Si.`
      });
